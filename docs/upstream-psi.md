...
| `harmonize-hct`             | KEEP (B)       | `Hct` overload of `Blend.harmonize`. Same overload-set split as `contrast-float`; `material-kolor`'s own `Blend.harmonize(Color, Color, Boolean)` extension resolves it through the implicit receiver.                                                                                                                                    |
